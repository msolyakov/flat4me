...
namespace Flat4me.Core.Rabbit
{
    ...
    public class RabbitFacade : IDisposable
    {
        ...
        private readonly string _exchange = RabbitFacadeConsts.EXCHANGE_DEFAULT;
        ...
        public RabbitFacade(string queueName, bool durable = true)
        {
            ...
            if (conn == null)
                throw new Exception("Unable to create RabbitFacade instance. Connection is null.", null);
        ...
        public void Publish(string contentType, Dictionary<string, object> headers, byte[] messageBody)
        {
            ContentType ctype = new ContentType(contentType);
            Publish(ctype, headers, messageBody);
        }
        ...
        /// <param name="messageBody">Массив байт с данными сообщения</param>
        public void Publish(ContentType contentType, Dictionary<string, object> headers, byte[] messageBody)
        {
            if (contentType == null)
        ...
        /// <param name="message">Объект Tuple, содержащий ContentType сообщения, заголовки сообщения (IDictionary), тело сообщения (byte[]), 
        ...
        public bool Next(int timeout, out Tuple<ContentType, IDictionary<string, object>, byte[], bool, ulong> message)
        {
            message = null;
            ...
            if (_consumer != null && _consumer.Queue.Dequeue(timeout, out ea))
            {
                ...
                message = new Tuple<ContentType, IDictionary<string, object>, byte[], bool, ulong>(ctype,
                    ea.BasicProperties.Headers, ea.Body, ea.Redelivered, ea.DeliveryTag);
